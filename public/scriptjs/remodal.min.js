! function(n, e) {
    "function" == typeof define && define.amd ? define(["jquery"], function(t) {
        return e(n, t)
    }) : "object" == typeof exports ? e(n, require("jquery")) : e(n, n.jQuery || n.Zepto)
}(this, function(t, s) {
    "use strict";
    var n, a, e, o = "remodal",
        i = t.REMODAL_GLOBALS && t.REMODAL_GLOBALS.NAMESPACE || o,
        r = s.map(["animationstart", "webkitAnimationStart", "MSAnimationStart", "oAnimationStart"], function(t) {
            return t + "." + i
        }).join(" "),
        d = s.map(["animationend", "webkitAnimationEnd", "MSAnimationEnd", "oAnimationEnd"], function(t) {
            return t + "." + i
        }).join(" "),
        c = s.extend({
            hashTracking: !0,
    ...
    function C(t, n, e) {
        var a = 0,
            ...
            o = function(t) {
                t.target === this && 0 == --a && (s.each(["$bg", "$overlay", "$wrapper", "$modal"], function(t, n) {
                    e[n].off(r + " " + d)
                }), n())
            };
        s.each(["$bg", "$overlay", "$wrapper", "$modal"], function(t, n) {
            e[n].on(r, i).on(d, o)
        }), t(), 0 === g(e.$bg) && 0 === g(e.$overlay) && 0 === g(e.$wrapper) && 0 === g(e.$modal) && (s.each(["$bg", "$overlay", "$wrapper", "$modal"], function(t, n) {
            e[n].off(r + " " + d)
        }), n())
    }
    ...
    function w(t, n) {
        var e = s(document.body),
            a = this;
        a.settings = s.extend({}, c, n), a.index = s[o].lookup.push(a) - 1, a.state = l.CLOSED, a.$overlay = s("." + O("overlay")), null !== a.settings.appendTo && a.settings.appendTo.length && (e = s(a.settings.appendTo)), a.$overlay.length || (a.$overlay = s("<div>").addClass(O("overlay") + " " + O("is", l.CLOSED)).hide(), e.append(a.$overlay)), a.$bg = s("." + O("bg")).addClass(O("is", l.CLOSED)), a.$modal = t.addClass(i + " " + O("is-initialized") + " " + a.settings.modifier + " " + O("is", l.CLOSED)).attr("tabindex", "-1"), a.$wrapper = s("<div>").addClass(O("wrapper") + " " + a.settings.modifier + " " + O("is", l.CLOSED)).hide().append(a.$modal), e.append(a.$wrapper), a.$wrapper.on("click." + i, "[data-" + o + '-action="close"]', function(t) {
            t.preventDefault(), a.close()
        }), a.$wrapper.on("click." + i, "[data-" + o + '-action="cancel"]', function(t) {
            t.preventDefault(), a.$modal.trigger(m), a.settings.closeOnCancel && a.close(m)
        }), a.$wrapper.on("click." + i, "[data-" + o + '-action="confirm"]', function(t) {
            t.preventDefault(), a.$modal.trigger(p), a.settings.closeOnConfirm && a.close(p)
        }), a.$wrapper.on("click." + i, function(t) {
            s(t.target).hasClass(O("wrapper")) && a.settings.closeOnOutsideClick && a.close()
        })
    }
    w.prototype.open = function() {
        var t, n = this;
        n.state !== l.OPENING && n.state !== l.CLOSING && ((t = n.$modal.attr("data-" + o + "-id")) && n.settings.hashTracking && (e = s(window).scrollTop(), location.hash = t), a && a !== n && y(a), a = n, function() {
            if (!f) {
                var t, n, e = s("html"),
                    a = O("is-locked");
                e.hasClass(a) || (n = s(document.body), t = parseInt(n.css("padding-right"), 10) + h(), n.css("padding-right", t + "px"), e.addClass(a))
            }
        }(), n.$bg.addClass(n.settings.modifier), n.$overlay.addClass(n.settings.modifier).show(), n.$wrapper.show().scrollTop(0), n.$modal.focus(), C(function() {
            $(n, l.OPENING)
        }, function() {
            $(n, l.OPENED)
        }, n))
        ...
        var n = this;
        n.state !== l.OPENING && n.state !== l.CLOSING && (n.settings.hashTracking && n.$modal.attr("data-" + o + "-id") === location.hash.substr(1) && (location.hash = "", s(window).scrollTop(e)), C(function() {
            $(n, l.CLOSING, !1, t)
        }, function() {
            n.$bg.removeClass(n.settings.modifier), n.$overlay.removeClass(n.settings.modifier).hide(), n.$wrapper.hide(), v(), $(n, l.CLOSED, !1, t)
        }, n))
    }, w.prototype.getState = function() {
        return this.state
    }, w.prototype.destroy = function() {
        var t = s[o].lookup;
        y(this), this.$wrapper.remove(), delete t[this.index], 0 === s.grep(t, function(t) {
            return !!t
        }).length && (this.$overlay.remove(), this.$bg.removeClass(O("is", l.CLOSING) + " " + O("is", l.OPENING) + " " + O("is", l.CLOSED) + " " + O("is", l.OPENED)))
    }, s[o] = {
        lookup: []
    }, s.fn[o] = function(e) {
        var a, i;
        return this.each(function(t, n) {
            null == (i = s(n)).data(o) ? (a = new w(i, e), i.data(o, a.index), a.settings.hashTracking && i.attr("data-" + o + "-id") === location.hash.substr(1) && a.open()) : a = s[o].lookup[i.data(o)]
        }), a
        ...
        }), s(document).find("." + i).each(function(t, n) {
            var e = s(n),
                a = e.data(o + "-options");
